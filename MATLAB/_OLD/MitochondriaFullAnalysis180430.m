...
try
    ...
    mitoLineProfLength = dataparam(1,3);
    actinLineProfLength = dataparam(1,1);
catch err
...
aisFiles = input('Are there AIS-files? (1=yes) ');
mitosPerFile = 1000;
...
for fileNum = fileNumbers
    ...
    filepathais = strFilepath(fileNum,filenameallAIS,masterFolderPath);
    ...
    try
        ...
        % NOW USES THE PROPER BINARY PATCHES MAP AS WELL, INSTEAD OF THE
        % NON-STRIPES MASK PREVIOUSLY USED. THIS IS THE SAME AS USED FOR
        % THE MITO-PATCHES OVERLAPPING AREA NOW. 
        imgrawnonstr = imread(filepathmask);
        imgrawpatches = imread(filepathpatches);
        %imgraw = bwareaopen(imgraw1,1500*pixelsize^2);
            ...
            try
                if ~isempty(imgais)
                    try
                        ...
                        if aisFiles
                            inAISparam = mitoAIS(datamito(i,1),datamito(i,2),pixelsize,imgais);
                            if inAISparam ~= 0
                                inAIS = [inAIS; i];
                            elseif inAISparam == 0
                                notInAIS = [notInAIS; i];
        ...
        if length(inAIS)>length(notInAIS)
            notInAIS = [notInAIS;zeros([length(inAIS)-length(notInAIS) 1])];
        elseif length(inAIS)<length(notInAIS)
            inAIS = [inAIS;zeros([length(notInAIS)-length(inAIS) 1])];
        end
        AISNumbers = [inAIS notInAIS];
        ...
        filesavenameais = strFilepath(fileNum,'_MitoInAIS.txt',masterFolderPath);
        ...
        dlmwrite(filesavenameais,AISNumbers,'delimiter','\t','precision',4);
...
filenameMitoNumAIS = '_MitoInAIS.txt';
...
for fileNum = fileNumbers
    ...
    filepathMitoNoStr = strFilepath(fileNum,filenameMitoNumStr,masterFolderPath);
    filepathMitoNoAIS = strFilepath(fileNum,filenameMitoNumAIS,masterFolderPath);
    filepathpxs = strFilepath(fileNum,filenameallPxs,masterFolderPath);
    ...
    try
        try
            ...
            dataMitoNoStr = dlmread(filepathMitoNoStr,'',1,1);
            sizeMitoNo = size(dataMitoNoStr);
        ...
        try
            dataMitoNoAIS = dlmread(filepathMitoNoAIS);
        catch err
        ...
        if not(isempty(dataMitoStr)) && not(isempty(dataStripes))
            dataMitoNoStr2 = transpose(nonzeros(dataMitoNoStr(1:end,1)));
            n=0;
            for i=dataMitoNoStr2
                n=n+1;
        ...
        elseif not(isempty(dataNonStripes))
            dataMitoNoNonStr2 = transpose(nonzeros(dataMitoNoStr(1:end,1)));
            n=0;
            for i=dataMitoNoNonStr2
                n=n+1;
                dataAnalysis(i,8)=dataWid(i,2); %Mito fitted width
                dataAnalysis(i,9)=dataWid(i,3); %Mito fitted center position
                dataAnalysis(i,10)=dataWid(i,5); %Distance to nearest non-stripes binary map patch
        ...
        if sizeMitoNo(2) ~= 1 && not(isempty(dataNonStripes))
            dataMitoNoNonStr2 = transpose(nonzeros(dataMitoNoStr(1:end,2)));
            n=0;
            for i=dataMitoNoNonStr2
                n=n+1;
                dataAnalysis(i,8)=dataWid(i,2); %Mito fitted width
        ...
        %%% AIS PARAMETER
        if not(isempty(dataAnalysis)) && not(isempty(dataMitoNoAIS))
            dataMitoNoAIS2 = transpose(nonzeros(dataMitoNoAIS(1:end,1)));
            for i=dataMitoNoAIS2
                dataAnalysis(i,14)=1; %Mito in the AIS
            end
%             dataMitoNoNonAIS2 = transpose(nonzeros(dataMitoNoAIS(1:end,2)));
%             for i=dataMitoNoNonAIS2
%                 dataAnalysis(i,14)=0; %Mito not in the AIS
%             end
        end
%         disp('ais done')
        ...
        if not(isempty(dataAnalysis))
            for i=1:sizeData(1)
                if dataAnalysis(i,4) ~= 0 && dataAnalysis(i,4) <= 0.05
                    dataAnalysis(i,20)=1; %Mito area smaller than 0.05µm^2
%                 else
%                     dataAnalysis(i,20)=0; %Mito area bigger than 0.05µm^2
                end
            end
        end
%         disp('mito small done')
...
%                     dataAnalysis(i,16)=0; %Mito smaller than actin width
        ...
        %%% MITO-ACTIN DISTANCE/DISPLACEMENT -
        ...
        if not(isempty(dataAnalysis))
            for i=1:sizeData(1)
                if dataAnalysis(i,8) ~= 0 && dataAnalysis(i,11) ~= 0
                    actinDist = NaN(1,3);
                    actinDistL = NaN(1,3);
                    actinDistR = NaN(1,3);
                    ...
                    dataAnalysis(i,24) = min(actinDist); %Mito-actin distance (negative = mito outside actin)
                    if dataAnalysis(i,24) < 0
                        dataAnalysis(i,25) = 1; %Mito outside actin filament
%                     else
%                         dataAnalysis(i,25) = 0; %Mito inside actin filament
                    end
                end
            end
        end
