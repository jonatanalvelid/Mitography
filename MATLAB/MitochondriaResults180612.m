...
masterFolderPath = strcat(uigetdir('X:\Mitography\NEW\AF22_HumanNeuro\Map488_Spectrin594_Tom635'),'\');
...
for fileNum=fileNumbers
    if fileNum < 10
        filenameStr = strcat('Image_00',int2str(fileNum),filenameActinStripes);
        filenameNonStr = strcat('Image_00',int2str(fileNum),filenameActinNonStripes);
    else
        filenameStr = strcat('Image_0',int2str(fileNum),filenameActinStripes);
        filenameNonStr = strcat('Image_0',int2str(fileNum),filenameActinNonStripes);
    end
    filepathStr = strcat(masterFolderPath,filenameStr);
    filepathNonStr = strcat(masterFolderPath,filenameNonStr);
    
    try
        data = dlmread(filepathStr);
        filamentStripes(:,:,fileNum) = data;
    catch err
        disp(strcat(num2str(fileNum),': No stripes data.'))
    end
    try
        data = dlmread(filepathNonStr);
        filamentNonStripes(:,:,fileNum) = data;
    catch err
        disp(strcat(num2str(fileNum),': No non-stripes data.'))
    end
end
%}
...
%{
    noProfiles = size(mitoWidthFiles);
    for i=1:noProfiles(1)
        if mitoWidth(i,2,fileNum) ~= 0
            mitoAllWidth = [mitoAllWidth; mitoWidth(i,2,fileNum)];
            mitoAllNonStripesDist = [mitoAllNonStripesDist; mitoNonStripesDist(i,2,fileNum)]; 
            if mitoNonStripesDist(i,2,fileNum) == 0
                noOfMitoInNonStripes = noOfMitoInNonStripes+1;
            else
                noOfMitoInStripes = noOfMitoInStripes+1;
            end
        end
    end
%}
    ...
    for i=1:mitosPerFile
        %{
        if filamentStripes(i,2,fileNum) ~= 0
            filamentWidthAllS = [filamentWidthAllS; filamentWidthFiles(i,2,fileNum)];
        end
        if filamentNonStripes(i,2,fileNum) ~= 0
            filamentWidthAllNS = [filamentWidthAllNS; filamentWidthFiles(i,2,fileNum)];
        end
        %}
...
%{
if isempty(mitoFilamentWidth)
    mitoFilamentWidth = [0 0];
    mitoArea = [0 0];
end
if isempty(mitoFilamentLength)
    mitoFilamentLength = [0 0];
end
if isempty(mitoWidthLength)
    mitoWidthLength = [0 0];
end
%}
...
interactivePlottingGUIThresholds180612(fontSizeGlobal, filamentWidthSNZ, mitoDisplacementEdgeS, mitoActinDistS, filamentWidthNSNZ, mitoDisplacementEdgeNS,...
