%%%
% Mitography - Turnover analysis - control - flipping
% Analysis the turnover ratio of mito proteins versus to the distance
% from the soma - in the cases where the distance is known
%----------------------------
% Version: 201110
...
filenameAISBinary = '_neuritesbinary_dt.tif';
%filenameTMR = '_vb.tif';
...
for fileNum = filenumbers
    ...
    try
        ...
        imagemitobinary = imclearborder(imagemitobinary);
        ...
        try
            aisdist = imread(filepathAISBinary);
            aisdist = double(aisdist);
        catch err
            % these are the images where we do not know where the soma is,
            % hence just put a very large distance to them (5000 pixels)
            aisdist = ones(size(imagemitobinary))*5000;
            aisdist = double(aisdist);
        end
