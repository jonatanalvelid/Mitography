...
%         circtemp(idxrem) = [];
        ...
        tmrsign = vertcat(tmrsign,sum(tmrsignaltemp));
        sirsign = vertcat(sirsign,sum(sirsignaltemp));
        area = vertcat(area,sum(areatemp));
        circ = vertcat(circ,mean(circtemp));
...
%figure(1)
hold on
s1 = scatter(ones(size(ydata)),ydata,20,'k.','jitter','on','jitterAmount',0.03); 
